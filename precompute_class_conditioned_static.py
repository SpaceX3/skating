...
from class_conditioned_retrieval import ClassConditionedRetriever
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build FS1000 class-conditioned static caches")
    ...
    parser.add_argument("--finefs-root", type=Path, default=Path("/home/v100/ZYQ/finefs_pocr_classifier-e11-c1"))
    parser.add_argument(
        "--c1-root",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/media/v100/disk3t/skating/fs1000_static_videomae_c1_class_retrieval"),
    )
    ...
    parser.add_argument("--probability-power", type=float, default=1.0)
    ...
    for ordinal, video_id in enumerate(video_ids, start=1):
        ...
        output_path = args.output_dir / (
            "static_videomae_c1_class_retrieval_{}_T{}.npy".format(video_id, dynamic_length)
        )
        if output_path.is_file() and not args.overwrite:
            values = np.load(output_path, mmap_mode="r")
            if values.shape != (dynamic_length, 1536):
        ...
        fused, retrieval_details = retriever.retrieve(
            query,
            probabilities,
            top_classes=args.top_classes,
            top_k=args.top_k,
            temperature=args.temperature,
            probability_power=args.probability_power,
            query_batch_size=args.retrieval_batch_size,
        )
        atomic_save(output_path, fused.astype(np.float16))
    ...
    summary = {
        "schema_version": "fs1000-class-conditioned-retrieval-v1",
        "class_order": list(CLASS_NAMES),
        "top_classes": args.top_classes,
        "top_k": args.top_k,
        "similarity": "cosine",
        "within_class_aggregation": "softmax",
        "temperature": args.temperature,
        "between_class_aggregation": "C1_probability",
        "probability_power": args.probability_power,
        "static_layout": "concat(query_768,knowledge_768)",
        "dtype": "float16",
        "videos": len(reports),
